...
@acknowledge_bp.route('/acknowledge_fixed')
        ...
        try:
            ...
            if current_shift_obj:
                ...
                inventory_group_query = Group.query.outerjoin(
                    ...
                    contains_eager(Group.inventories, alias=fixed_inventory_alias) 
                ).order_by(Group.id)
...
@acknowledge_bp.route('/acknowledge_ent', methods=['POST'])
def acknowledge_ent():
    date_str = request.form.get('date')
    shift_str = request.form.get('shift')
    # users = User.query.order_by(User.username).all() # Not passed to ent template, but dental needs it
    ...
    if date_str and shift_str:
        try:
            ...
            if current_shift_obj:
                ent_items_query = Inventory.query.filter_by(fixed=False, groupid=2).order_by(Inventory.id).all() # Assuming groupid 2 is ENT
    ...
    return render_template('acknowledge_ent.html', date=date_str, shift=shift_str, 
                           ent_inventory=ent_inventory_with_quantities) # users not needed here
...
def acknowledge_dental():
    ...
    if date_str and shift_str:
        try:
            ...
            if current_shift_obj:
                dental_items_query = Inventory.query.filter_by(fixed=False, groupid=3).order_by(Inventory.id).all() # Assuming groupid 3 is Dental
